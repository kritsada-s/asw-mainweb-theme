...
<section class="search-section" data-count="<?=$allsearch->post_count?>">
	<div class="search-section-top">
		<div class="-l">
			<h2 class="search-count-text"><?php pll_e('แอสเซทไวส์คลับ')?> <span class="search-count-top">- <span class="search-count-num"><?=$allsearch->post_count?></span> <?php pll_e('รายการ')?></span></h2>
		</div>
		<div class="-r">
			<a  class="see-more selected-all f30-28" href="/<?=pll_current_language()?>/?s=<?=$keyword?>&focus=club">
		...
		while ( $allsearch->have_posts() AND $con < 3 OR $s_style == 'long' AND $con < $allsearch->post_count) : $allsearch->the_post();
			...
			<div class="col-span-1 news-item-0 relative" data-aos="fade-up" data-aos-duration="500" data-aos-delay="<?=$con%3?>00">
				<a href="<?= get_permalink() ?>" class="">
					...
					<div class="bg-ci-grey-900 h-full px-4 py-6 relative pointer" onclick="">
						<?php if ($con%3!=1): ?>
...
<script type="text/javascript">$(`[data-value="club"]`).dataset.count = <?=$allsearch->post_count?></script>