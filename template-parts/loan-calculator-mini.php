...
<div id="loan_calc_section" class="">
  <div class="px-5">
    <div class="flex flex-col items-center w-full">
      <h2 class="text-3xl md:text-4xl font-medium lg:text-5xl mx-auto text-[#124375] mb-2 md:mb-5">โปรแกรมคำนวณสินเชื่อ</h2>
      <div id="calc_tabs" class="">
          ...
          <div class="tab-content active" data-tab="1">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
              ...
              <div class="lg:p-6 order-2 lg:order-1">
                <form id="loanCalculatorForm" class="flex flex-col gap-4">
                  <div class="flex flex-col gap-2">
                    <label for="net_income" class="text-lg mb-0 text-neutral-700"><?php pll_e('รายได้ต่อเดือน (รวมรายได้ผู้กู้ร่วม (ถ้ามี))')?></label>
                    <div class="input-group relative">
                      <input type="text" id="net_income" name="net_income" pattern="\d*" class="!w-full border-t-0 border-x-0 border-b border-neutral-300 pl-2 pr-10 py-2 text-xl focus:outline-none focus:border-[#124375] leading-none" value="25,000" required>
                      <span class="text-neutral-500 absolute right-0 top-0 bottom-0 flex items-center"><?php pll_e('บาท')?></span>
                    </div>
                  </div>

                  <div class="flex flex-col gap-2">
                    <label for="net_debt" class="text-xl text-neutral-700"><?php pll_e('ภาระหนี้ที่ต้องผ่อนชำระต่อเดือน (ถ้ามี)')?></label>
                    <div class="input-group relative">  
                      <input type="text" id="net_debt" pattern="\d*" name="net_debt" class="!w-full border-t-0 border-x-0 border-b border-neutral-300 pl-2 pr-10 py-2 text-xl focus:outline-none focus:border-[#124375] leading-none" value="8,000" required>
                      <span class="text-neutral-500 absolute right-0 top-0 bottom-0 flex items-center"><?php pll_e('บาท')?></span>
                    </div>
                  </div>

                  <div class="flex flex-col gap-2">
                    <label for="period" class="text-xl text-neutral-700"><?php pll_e('ระยะเวลาที่ขอกู้ (ปี)')?></label>
                    <div class="input-group relative">
                      <input type="number" id="period" name="period" min="1" max="30" class="!w-full border-t-0 border-x-0 border-b border-neutral-300 pl-2 pr-10 py-2 text-xl focus:outline-none focus:border-[#124375] leading-none" value="30" max="30" required>
                      <span class="text-neutral-500 absolute right-0 top-0 bottom-0 flex items-center"><?php pll_e('ปี')?></span>
                    </div>
                  </div>

                  <div class="flex gap-4 mt-4">
                    <button type="button" id="calculateBtn" onclick="calculateLoan()" class="flex-1 px-6 py-2 bg-[#124375] text-white font-medium text-xl md:text-3xl hover:bg-[#0e345d] transition-all duration-300">
                      <?php pll_e('คำนวณ')?>
                    </button>
                    <button type="button" onclick="clearForm()" class="flex-1 px-6 py-2 bg-neutral-100 text-neutral-700 text-xl md:text-3xl font-light hover:bg-neutral-200 transition-all duration-300">
              ...
              <div class="bg-[#124375] order-1 lg:order-2 p-4 lg:p-6 shadow-[7px_7px_0_rgba(0,0,0,0.5)] lg:shadow-[10px_10px_0px_rgba(0,0,0,0.5)] flex flex-col gap-2 justify-center mb-4 lg:mb-0">
                <h3 class="text-xl font-medium text-neutral-300 text-center"><?php pll_e('คำนวณวงเงินที่สามารถกู้ได้')?></h3>
                <div id="calculationResults" class="flex flex-col gap-4">
                  <h3 class="text-white text-5xl text-center font-medium animate__animated flex justify-center items-baseline gap-2 leading-[0.7]">
                    <span id="loan_result" data-target="3,300,000">3,300,000</span>
                    <span class="text-white text-center text-[20px]"><?= pll_e('บาท*') ?></span>
                  </h3>
                  <p class="text-neutral-100 text-center text-[14px] md:text-[16px] font-light lg:block"><?= pll_e('*การคำนวณนี้เป็นการประมาณการยอดเงินกู้ได้สูงสุดเพื่อที่อยู่อาศัย ซึ่งขึ้นอยู่กับรายได้ต่อปีของคุณและความสามารถในการบริการสินเชื่อ') ?></p>
                </div>
              </div>
            </div>
          </div>

          <!-- Tab 2 :: Monthly Payment Calculator -->
          <div class="tab-content" data-tab="2">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <!-- Left Column - Form -->
              <div class="lg:p-6 order-2 lg:order-1">
                <form id="loanCalculatorForm" class="flex flex-col gap-2">
                  <div class="flex flex-col gap-2">
                    <label for="net_loan" class="text-lg mb-0 text-neutral-700"><?php pll_e('จำนวนเงินที่ขอกู้')?></label>
                    <div class="input-group relative">
                  ...
                  <div class="flex flex-col gap-2">
                    <label for="net_debt" class="text-lg text-neutral-700"><?php pll_e('อัตราดอกเบี้ยต่อปี')?></label>
                    <div class="input-group relative">  
                  ...
                  <div class="flex flex-col gap-2">
                    <label for="dept_period" class="text-lg text-neutral-700"><?php pll_e('ระยะเวลาที่ขอกู้ (ปี)')?></label>
                    <div class="input-group relative">
                  ...
                  <div class="flex gap-4 mt-4">
                    <button type="button" id="calculateBtn" onclick="calculateDept()" class="flex-1 px-6 py-2 bg-[#124375] text-white font-medium text-lg md:text-3xl hover:bg-[#0e345d] transition-all duration-300">
                      <?php pll_e('คำนวณ')?>
                    </button>
              ...
              <div class="bg-[#124375] order-1 lg:order-2 p-4 lg:p-6 shadow-[7px_7px_0_rgba(0,0,0,0.5)] lg:shadow-[10px_10px_0px_rgba(0,0,0,0.5)] flex flex-col gap-2 justify-center mb-4 lg:mb-0">
                ...
                <div id="calculationDeptResults" class="flex flex-col gap-4">
                  <h3 class="text-white text-5xl text-center font-medium animate__animated flex justify-center items-baseline gap-2 leading-[0.7]">
                    ...
                    <span class="text-white text-center text-[20px]">บาท*</span>
                  </h3>
                  <p class="text-neutral-100 text-center text-[16px] md:text-[18px] font-light lg:block">*ผลการคำนวณข้างต้นเป็นเพียงการคำนวณเบื้องต้นเท่านั้น
