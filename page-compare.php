...
<link rel="stylesheet" type="text/css" href="/wp-content/themes/seed-spring/css/compare.css?t=?<?=time()?>">
<?php 
...
$type_list = 0;
$location_list_bkk = get_term_by('slug', 'in-bangkok', 'project_location')->term_id;
$location_list_upcountry = get_term_by('slug', 'upcountry', 'project_location')->term_id;
$brand_list_condo = get_term_by('slug', 'condominium', 'project-type')->term_id;
$brand_list_house = get_term_by('slug', 'house', 'project-type')->term_id;

if ($location_list_bkk == '') {
	$location_list_bkk = 0;
}
if ($location_list_upcountry == '') {
	$location_list_upcountry = 0;
}
if ($brand_list_condo == '') {
	$brand_list_condo = 0;
}
...
if ($pj_compare_slug>0) {
	if ($pj_compare_slug[0] != '') {
		foreach ($pj_compare_slug as $key => $slug) {
			...
			if ($posts[0]->ID != '') {
				$pj_id = $posts[0]->ID;
				$pj_thisl_lang = pll_get_post( $pj_id, pll_current_language() );
				$pj_id = $pj_thisl_lang;
				if ($pj_id != '') {
...
$top_url_param = "/".pll_current_language()."/compare/?project=".$top_param;
if (!$found_all) {
	?>
	<script type="text/javascript">
...
<div id="app" class="container mx-auto compare">
	...
	<div class="comp-bg">
		<div class="comp-modal" data-filter-toggle="-1" @click="checkIfFilterToggle">
			...
			<div class="-inner">
				<h2 class="popup-filter-title-top mb-5 mt-4"><?php pll_e('เลือกโครงการเพื่อเปรียบเทียบ') ?></h2>

				<!-- Mobile -->
				<section class="filter-mb-section" @click="mbExpandToggle()">
					<img src="<?= ASW_MEDIA_PATH ?>/2022/10/Icon-in-input.png" class="inline-block" style="height:2rem;">
					<span><?php pll_e('ค้นหาที่อยู่โครงการในแบบคุณ') ?></span>
					<img src="<?= ASW_MEDIA_PATH ?>/2022/11/arrow.png" class="quick-filter-arrow" style="height: 15px">	
				</section>
				...
				<div v-if="mbExpand" class="filter-mb-expand">
					...
					<div class="filter-mb-type">
						<div class="filter-menu" @click="selectToggle('type')">
							...
							<img src="<?= ASW_MEDIA_PATH ?>/2022/10/Vector.png" class="quick-filter-arrow" :class="{ 'rotate180deg' : state == 'type' }" style="height:.5rem;margin: 0;">
						</div>
					...
					<div class="filter-mb-type">
						<div class="filter-menu" @click="selectToggle('location')">
							...
							<img src="<?= ASW_MEDIA_PATH ?>/2022/10/Vector.png" class="quick-filter-arrow" :class="{ 'rotate180deg' : state == 'location' }" style="height:.5rem;margin: 0;">
						</div>
					...
					<div class="filter-mb-type">
						<div class="filter-menu" @click="selectToggle('brand')">
							...
							<img src="<?= ASW_MEDIA_PATH ?>/2022/10/Vector.png" class="quick-filter-arrow" :class="{ 'rotate180deg' : state == 'brand' }" style="height:.5rem;margin: 0;">
						</div>
					...
					<div class="filter-mb-type">
						<div class="filter-menu" @click="selectToggle('price')">
							...
							<img src="<?= ASW_MEDIA_PATH ?>/2022/10/Vector.png" class="quick-filter-arrow" :class="{ 'rotate180deg' : state == 'price' }" style="height:.5rem;margin: 0;">
						</div>
					...
					<div class="filter-box">
						<div @click="selectToggle('type')">
							...
							<span v-if="filter_type.length <= 0"><?php pll_e('ประเภทโครงการ') ?></span>
								...
								<img src="<?= ASW_MEDIA_PATH ?>/2022/10/Vector.png" class="quick-filter-arrow" :class="{ 'rotate180deg' : state == 'type' }" style="height:.5rem">
							</div>
							<div @click="selectToggle('location')">
								...
								<span v-if="filter_location.length <= 0"><?php pll_e('ทำเลที่คุณสนใจ') ?></span>
									...
									<img src="<?= ASW_MEDIA_PATH ?>/2022/10/Vector.png" class="quick-filter-arrow" :class="{ 'rotate180deg' : state == 'location' }" style="height:.5rem">
								</div>
								<div @click="selectToggle('brand')">
									...
									<span v-if="filter_brand.length <= 0"><?php pll_e('เลือกแบรนด์') ?></span>
										...
										<img src="<?= ASW_MEDIA_PATH ?>/2022/10/Vector.png" class="quick-filter-arrow" :class="{ 'rotate180deg' : state == 'brand' }" style="height:.5rem">
									</div>
									<div @click="selectToggle('price')">
										<img src="<?= ASW_MEDIA_PATH ?>/2022/10/Icon-in-input-3.png" class="inline-block" style="height:2rem;">
										<span v-if="filter_price.label == 'none'"><?php pll_e('ช่วงราคา') ?></span>
										<span v-else>{{ filter_price.label }}</span>
										<img src="<?= ASW_MEDIA_PATH ?>/2022/10/Vector.png" class="quick-filter-arrow" :class="{ 'rotate180deg' : state == 'price' }" style="height:.5rem">
									</div>
								</div>
							...
							<h2 class="popup-filter-title"><?php pll_e('โครงการทั้งหมด') ?></h2>
							<br/>
							...
							<div class="estate-list" the="<?=$cate_parent->name?>" cate="<?=$cate_brand->name?>">
								...
								while ( $loop->have_posts() ) : $loop->the_post(); {
									...
									if ($post->post_name != 'en') {
										...
										{{ storeData('<?=$post->ID?>', '<?=$cate_parent->name?>', '<?=$cate_parent->slug?>', '<?=$loca_child->name?>', '<?=$post->post_title?>') }}
					...
					<div v-if="selectEstate.length > 0" class="compare-mobile-short-container">
						<div v-for="estate in selectEstate" :key="'cp_pc' + estate" class="compare-mobile-short">
							...
							<div class="detail-set">
								<div>
									<span v-if="findEstateData(estate).type_slug == 'condominium'">
		...
		<script type="text/javascript">
			...
			foreach ($pj_data as $pj_id => $data) {
				...
				<a class="read-more" href="<?=$data['post']->guid?>" target="_blank"><?php pll_e('ดูรายละเอียด') ?></a></div>
				`
			...
			const vestate = createApp({
				setup() {
					...
					const type_list = ref(<?php echo json_encode(get_terms(array(
						...
						'hide_empty' => false,
						)))?>.filter((data) => data.parent ==<?=$type_list?>))
					const location_list_bkk = ref(<?=json_encode(get_terms(array(
						'taxonomy' => 'project_location',
						'hide_empty' => false,
						)))?>.filter((data) => data.parent == <?=$location_list_bkk?>))
					const location_list_upcountry = ref(<?=json_encode(get_terms(array(
						'taxonomy' => 'project_location',
						'hide_empty' => false,
						)))?>.filter((data) => data.parent == <?=$location_list_upcountry?>))
					const brand_list_condo = ref(<?=json_encode(get_terms(array(
						'taxonomy' => 'project-type',
						'hide_empty' => false,
						)))?>.filter((data) => data.parent == <?=$brand_list_condo?>))					
					const brand_list_house = ref(<?=json_encode(get_terms(array(
						'taxonomy' => 'project-type',
						'hide_empty' => false,
						)))?>.filter((data) => data.parent == <?=$brand_list_house?>))
					...
					function storeData(id, type, type_slug,location, name) {
						allEstateStore.value.push({
							id, name, type,type_slug, location,type_slug
						});
					}
					...
					function onShow(type, location, brand, price){
						...
						if(this.filter_price.label != 'none'){
							if(price*10**6 >= this.filter_price.min  && price*10**6 <= this.filter_price.max*10**6){
								check_price = true;
							} else {
								check_price = false;
							}
