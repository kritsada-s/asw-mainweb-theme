...
<?php 
$f = get_fields();
$s_type_title = pll__('คอนโดมิเนียม');
$s_type_url = 'condominium';
	...
	<div class="cont-pd t-center">
		...
		<div class="grid grid-cols-2 md:flex md:flex-wrap md:justify-center gap-4 md:gap-6 px-1 sm:px-4 lg:px-40">
			...
			foreach ($term_pj_type['condominium']->child as $key => $value) {
				...
				$condo_link = get_term_link($value->term_id);
				if ($is_show != 'hide') {
					?>
...
<div style="overflow: visible;">
	<section id="condo-filter" class="pt-16 pb-6" style="background: linear-gradient(180deg, #EDF2F7 -4.71%, #FFFFFF 164.99%);">
		<h1 class="flex justify-center f40-30 cl-ci-grey-200" style="white-space: nowrap;"><?php pll_e('ค้นหาโครงการคอนโดมิเนียม')?></h1>
		<sp class="xl"></sp>
		<sp class="xl"></sp>
...
<script type="text/javascript">
	...
	function sort_info(){
		...
		for(let i of allcard){
			...
			xconsolex.log('set_filter_price_min',set_filter_price_min)
			xconsolex.log('set_filter_price_max',set_filter_price_max)
	...
	<div class="cont-pd">
		...
		<div class="grid grid-rows-2 md:grid-rows-1 md:grid-cols-2 flex items-center">
			<div class="row-span-1 md:col-span-1">
				<h5 class="f30-28 cl-ci-grey-100"><?php pll_e('คอนโดมีเนียมทั้งหมด')?> <span id="num-con" class="cl-ci-grey-400">- <?=$count_num['condominium']?> <?php pll_e('รายการ')?></span></h5>
			</div>
